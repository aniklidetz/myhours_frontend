...
export default function PayrollScreen() {
  ...
  const [_currentSalaryData, setCurrentSalaryData] = useState(null);
  ...
  const { palette: _palette, isDark: _isDark } = useColors();
  ...
  // Main initialization effect - simplified to avoid duplicate API calls
  useEffect(() => {
    ...
    // Employee fetching is handled in the separate effect below
  }, [canViewAllEmployees, user, theme]);
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async (retryCount = 0) => {
      console.log(' [PAYROLL DEBUG] Starting fetchEmployees...');
      console.log(' [DEBUG] User:', user?.username, 'CanViewAll:', canViewAllEmployees);

      if (!canViewAllEmployees) {
        console.log(' [DEBUG] User cannot view all employees, skipping fetch');
        return;
      }
      ...
      try {
        ...
        const response = await ApiService.employees.getAll({ page_size: 100 });

        console.log(' [DEBUG] Payroll response:', response ? 'SUCCESS' : 'NO_RESPONSE');
        console.log(' [DEBUG] Response type:', Array.isArray(response) ? 'ARRAY' : typeof response);
        console.log(
          ' [DEBUG] Payroll results:',
        ...
        if (Array.isArray(response)) {
          ...
          console.log(' [DEBUG] Using array response directly:', employeesList.length, 'employees');
        } else if (response && response.results) {
          employeesList = response.results;
          console.log(' [DEBUG] Using response.results:', employeesList.length, 'employees');
        }

        if (employeesList.length > 0) {
          const employeeList = employeesList.map(emp => ({
            id: emp.id,
        ...
        if (error.response?.status === 401 && retryCount < 2) {
          safeLog(' Authentication error fetching employees, retrying...', { retryCount });
          setTimeout(() => {
      ...
      try {
        ...
        if (canViewAllEmployees && !selectedEmployee) {
          ...
          if (Array.isArray(response)) {
            dataArray = response;
            console.log(
              ' Received array response for all employees:',
              dataArray.length,
              'employees'
            );
          } else {
    ...
    const debounceTimer = setTimeout(fetchData, 500); // Increased delay to account for employee fetch
  ...
  // Debug mode only for development and when explicitly not disabled
  const _isDebugMode = __DEV__ && !process.env.DISABLE_PAYROLL_DEBUG;
  ...
  const handleConfirm = id => {
    showGlassConfirm(
      ...
      () => {
        setPayrollData(
          payrollData.map(item => (item.id === id ? { ...item, status: 'Confirmed' } : item))
        );
  ...
  const _renderPayrollItem = ({ item }) => {
    return (
      <View style={stylesWithDarkMode.simpleCard}>
        <Text style={stylesWithDarkMode.simpleCardText}>{item.employee.name}</Text>
        <Text style={stylesWithDarkMode.simpleCardSubtext}>{item.period}</Text>
        <Text style={stylesWithDarkMode.simpleCardSubtext}>Total: {item.totalPayout} ₪</Text>
        <Text style={stylesWithDarkMode.simpleCardSubtext}>Hours: {item.hoursWorked}h</Text>
      </View>
    );
  };
    ...
    <View style={stylesWithDarkMode.emptyContainer}>
      ...
      {selectedPeriod && (
        <Text style={stylesWithDarkMode.emptySubtext}>
          Showing data for:{' '}
          {availablePeriods.find(p => p.key === selectedPeriod)?.label || selectedPeriod}
        </Text>
      )}
    </View>
  );
      ...
      <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={[
                        ...
                        selectedEmployee?.id === emp.id &&
                          stylesWithDarkMode.selectorButtonTextActive,
                      ]}
                    >
                      {emp.name}
                    </Text>
        ...
        {loading ? (
          <ActivityIndicator
            size="large"
            color={theme.colors.text.primary}
            style={stylesWithDarkMode.loader}
          />
                      ...
                      <View style={{ alignItems: 'flex-end' }}>
                        ...
                        >
                          {item.totalWorkingDays > 0
                            ? Math.round((item.workedDays / item.totalWorkingDays) * 100)
                            : 0}
                          %
                        </Text>
                      </View>
                ...
                >
                  <View>
                    ...
                    >
                      {item.hourlyRate > 0
                        ? `${193}h`
                        : `${item.workedDays || 0}/${item.totalWorkingDays || 0}`}
                    </Text>
                  </View>
                  <View style={{ alignItems: 'center' }}>
                    <Text
