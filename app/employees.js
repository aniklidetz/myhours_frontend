...
import { safeLogWorkLogs } from '../src/utils/safeLogging';
...
export default function EmployeesScreen() {
  ...
  const [_requiresBiometric, setRequiresBiometric] = useState(false);
  ...
  const { user, hasAccess: _hasAccess, logout, loading: userLoading } = useUser();
  const { palette: _palette } = useColors();
  const theme = useLiquidGlassTheme();
  const { officeSettings } = useOffice();
  const { location: _location, isUserInRadius } = useLocation({ watchPosition: false });
  const {
    workStatus,
  ...
  // Function to calculate hours - MOVED BEFORE EARLY RETURN
  const calculateTodayHours = useCallback(async () => {
    ...
    try {
      ...
      const workLogs = await ApiService.worktime.getLogs({
        ...
        page_size: 50, // Limit the number of records
      });
      ...
      if (workLogs && workLogs.results && workLogs.results.length > 0) {
        const todayLogs = workLogs.results.filter(log => {
          ...
          const checkOutDate = log.check_out
            ? new Date(log.check_out).toISOString().split('T')[0]
            : null;
        ...
        console.log(
          ` Filtered logs for current user: ${todayLogs.length}/${workLogs.results.length}`
        );

        todayLogs.forEach(log => {
          const hoursWorked = log.total_hours || log.hours_worked;
          console.log(
            ` Processing log: hours=${hoursWorked}, check_in=${log.check_in}, check_out=${log.check_out}`
          );
      ...
      if (error.message?.includes('Network Error') || error.message?.includes('timeout')) {
        console.warn(' Network unavailable or timeout, using cached hours');
        setTodayHours('--');
      } else if (error.code === 'ECONNABORTED' || error.message?.includes('exceeded')) {
        console.warn(' Request timeout exceeded, retrying with cached data');
        setTodayHours('--');
      } else {
  ...
  // ALL EFFECTS AND CALLBACKS MOVED BEFORE EARLY RETURN
  ...
  useFocusEffect(
    ...
    }, [user, loadWorkStatus, calculateTodayHours, loadEnhancedAuthStatus, loadUserBiometricStatus])
  );
  ...
  const _getRoleDisplayName = role => {
    switch (role) {
      case ROLES.ADMIN:
      ...
      {user?.role && (
        <View style={styles(theme).roleBadgeContainer}>
          <View style={styles(theme).roleBadge}>
            <Text style={styles(theme).roleBadgeText}>{user.role}</Text>
          </View>
        </View>
      )}
          ...
          >
            ...
            <View
              style={[
                ...
                workStatus === 'on-shift'
                  ? styles(theme).onShiftBadge
                  : styles(theme).offShiftBadge,
              ]}
            >
            ...
            {workStatus === 'on-shift' && (
              <Text style={styles(theme).shiftTimeText}>Shift in progress</Text>
            )}
          </LiquidGlassCard>
            ...
            <View style={styles(theme).actionRow}>
              ...
              >
                ...
                <Ionicons
                  ...
                  size={14}
                  color={theme.colors.text.secondary}
                  style={styles(theme).actionChevron}
                />
              </LiquidGlassCard>
              ...
              {userBiometricStatus && (
                <>
                  {!userBiometricStatus.has_biometric ? (
                    ...
                    <View
                      style={[styles(theme).actionCard, styles(theme).completedCard]}
                      accessibilityRole="text"
