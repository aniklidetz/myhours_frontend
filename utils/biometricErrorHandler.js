  ...
  switch (status) {
    case 400:
      ...
      if (responseData?.error?.includes('No registered faces')) {
        return {
          ...
          message: 'No employees have registered biometric data yet. Please register your face first.',
          icon: '👤'
        };
      }
      ...
      if (responseData?.error?.includes('Face recognition failed')) {
        return {
          ...
          message: 'Unable to recognize your face. Please ensure good lighting and try again. If this continues, you may need to re-register your biometric data.',
          icon: '👤'
        };
      }
      return {
    ...
    case 403:
      ...
      if (responseData?.error?.includes('Face does not match')) {
        return {
          title: 'Face Mismatch',
          message: 'The face doesn\'t match your account. Please ensure you\'re logged in with the correct account.',
          icon: '🚫'
        };
      }
    ...
    case 404:
      ...
      return {
        title: 'Face Not Recognized',
        message: 'Your face is not registered in the system. Please register your biometric data first.',
        icon: '🔍'
      };
    ...
    case 409:
      ...
      return {
        ...
        message: 'Biometric data is already registered for this employee. You can update it if needed.',
        icon: '♻️'
      };
    ...
    case 504:
      ...
      return {
        ...
        message: 'The biometric system is temporarily unavailable. Please try again in a few minutes or contact support.',
        icon: '🔧'
      };
...
export const showBiometricError = (error, operation, showAlert, showToast) => {
  ...
  const isFaceRecognitionError = errorInfo.title.includes('Face') || 
  ...
  const isCritical = [403, 404, 500, 502, 503, 504].includes(error.response?.status) || 
                     isFaceRecognitionError;
