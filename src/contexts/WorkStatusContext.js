...
const WORK_STATUS_KEY = APP_CONFIG.STORAGE_KEYS.WORK_STATUS;
...
export const WorkStatusProvider = ({ children }) => {
  ...
  const [isLoadingRef, setIsLoadingRef] = useState(false); // Additional throttling
  const { user } = useUser();

  // Load work status from backend
  const loadWorkStatus = useCallback(
    async (forceRefresh = false) => {
      ...
      if (isLoadingRef) {
        console.log('Work status already loading, skipping');
        return;
      }
      ...
      try {
        ...
        if (tokenExpiration.isExpired || tokenExpiration.shouldRefresh) {
          try {
            ...
            console.warn('Token refresh failed, using cached data:', refreshError.message);
            // Don't spam logs with full error objects
            return;
          }
        }
      } catch (_error) {
        console.warn('Could not check token expiration, skipping API call');
        return;
      }

      // Prevent too frequent updates (unless forced)
      const now = Date.now();
      if (!forceRefresh && lastUpdate && now - lastUpdate < 5000) {
        // 5 seconds
        console.log('Skipping work status update (too soon)');
        return;
      }

      try {
        setLoading(true);
        setIsLoadingRef(true); // Set loading flag
        ...
        const response = await Promise.race([ApiService.biometrics.checkStatus(), timeoutPromise]);
        ...
        if (response.success || response.is_checked_in !== undefined) {
          ...
          console.log('Work status loaded:', {
            ...
            employeeName: response.employee_info?.employee_name
              ? maskName(response.employee_info.employee_name)
              : 'unknown',
          });
        ...
        if (
          ...
          console.warn('Network unavailable or timeout, using cached data');
        } else if (error.code === 'ECONNABORTED') {
          console.warn('Request timeout exceeded, using cached data');
        } else {
        ...
        setLoading(false);
        setIsLoadingRef(false); // Clear loading flag
      }
    },
    [user]
  ); // Remove lastUpdate from dependencies
  ...
  const handleCheckInSuccess = useCallback(
    async checkInData => {
      ...
      if (user && user.id) {
        ...
        console.warn('No user found for AsyncStorage save');
      }

      // Clear employees cache to refresh hours data
      try {
        console.log('Clearing employees cache after check-in...');
        ...
        console.warn('Cache clear failed:', _error);
      }
  ...
  const handleCheckOutSuccess = useCallback(
    ...
    [user]
  ); // Remove loadWorkStatus from dependencies
  ...
  }, [workStatus]); // Remove loadWorkStatus from dependencies
