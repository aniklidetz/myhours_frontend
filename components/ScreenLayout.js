...
function ScreenLayout({
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={edges}>
      <LiquidGlassLayout>{containerContent}</LiquidGlassLayout>
    </SafeAreaView>
  );
}
...
const ScreenLayoutWithHeader = ({ title, subtitle, children, ...props }) => {
  const theme = useLiquidGlassTheme();
  ...
  const header = (
    <View style={headerStyles.header}>
      {typeof title === 'string' ? <Text style={headerStyles.title}>{title}</Text> : title}
...
// Assign components to ScreenLayout object
ScreenLayout.WithHeader = ScreenLayoutWithHeader;
ScreenLayout.WithForm = ScreenLayoutWithForm;
